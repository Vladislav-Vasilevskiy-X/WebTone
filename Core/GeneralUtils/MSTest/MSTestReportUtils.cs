...
namespace Core.GeneralUtils.MSTest
{
	...
	public static class MSTestReportUtils
	{
		...
		public static void AddIssueToReport(TestContext testContext)
		{
			...
			var currentlyRunningClassType =
				Assembly.GetCallingAssembly().GetTypes().FirstOrDefault(f => f.FullName == theClassName);
			if (currentlyRunningClassType != null)
			{
				var currentlyRunningMethod = currentlyRunningClassType.GetMethod(testName);
				var issueAttributes =
					currentlyRunningMethod.GetCustomAttributes(typeof(IssueAttribute), true) as IEnumerable<IssueAttribute>;
				if (issueAttributes != null)
				{
					var firstOne = issueAttributes.FirstOrDefault();
					if (firstOne != null)
					{
						Assert.Inconclusive("Test failed by issue: Id: {0}, Description: {1}, Link: {2}", firstOne.Id,
							firstOne.Description, firstOne.Link);
					}
				}
			}
		}
		...
		/// <summary>
		///     As Mstest has issue with formatting of test message and throws <see cref="FormatterException" /> when test message
		///     contain placeholders like '{0' or '2}'
